        ...
        if(!query.exec())
         ...
         while (query.next())
         {
             if(doToc == false)
             {
                 ...
                 jsonObject["comments_count"] = QString::number( StandardsDataComments::getSpecsCommentsCount(id.toInt() ));
...
/*
QString temp = QString::fromStdString(to);
temp.replace(QRegExp(" style=.*\""), "");
//temp.replace(QRegExp("/h1>."), "/h1>\n");
to = temp.toStdString();
*/
...
QMap<QString, QString> StandardsData::sqlObjCatalog(QString name, QString ac, QString pc, QString country, QString lang, QString cat, QString spec_active, QString release)
{
    ...
    query.prepare("select lfdnr, id from standards_data where obj_sname LIKE ? and ac_classes LIKE ? and pc_classes LIKE ? and lang = ? and country = ? and cat_class = ? AND spec_active = ? and (spec_release = ? or spec_release = 'pre-release') order by lfdnr");
...
QJsonArray StandardsData::listObjCatalog(bool doToc, QMap<QString, QString> editMap)
{
    ...
    if(editMap["obj_sname"].compare("General") == 0)
    {
        ...
        localList= StandardsData::sqlObjCatalog("General", "0", "0", editMap["country"], editMap["lang"], "General", editMap["spec_active"], editMap["spec_release"]);
        wwList = StandardsData::sqlObjCatalog("General", "0", "0", "WW", editMap["lang"], "General", editMap["spec_active"], editMap["spec_release"]);
    ...
    {
        wwList.clear(); localList.clear(); outList.clear();
        localList= StandardsData::sqlObjCatalog(editMap["obj_sname"], "0", "0", editMap["country"], editMap["lang"], "General", editMap["spec_active"], editMap["spec_release"]);
        wwList = StandardsData::sqlObjCatalog(editMap["obj_sname"], "0", "0", "WW", editMap["lang"], "General", editMap["spec_active"], editMap["spec_release"]);
        outList = StandardsData::checkObjCatalog(wwList, localList);
        array = StandardsData::sqlGet_crObjCatalog(doToc, outList);

        wwList.clear(); localList.clear(); outList.clear();
        localList= StandardsData::sqlObjCatalog(editMap["obj_sname"], editMap["ac_class"], editMap["pc_class"], editMap["country"], editMap["lang"], "General", editMap["spec_active"], editMap["spec_release"]);
        wwList = StandardsData::sqlObjCatalog(editMap["obj_sname"], editMap["ac_class"], editMap["pc_class"], "WW", editMap["lang"], "General", editMap["spec_active"], editMap["spec_release"]);
        outList = StandardsData::checkObjCatalog(wwList, localList);
...
QJsonArray StandardsData::upReleased(int id)
{
...
void StandardsData::setDraft(int id, QString comment)
{
    TSqlQuery query;

    query.prepare("UPDATE public.standards_data SET spec_release='draft' WHERE id=?");
    query.addBindValue(id);
    query.exec();

    query.prepare("UPDATE public.standards_meta SET spec_comment=? WHERE spec_data_id=?");
    query.addBindValue(comment);
    query.addBindValue(id);
    query.exec();
}

QJsonArray StandardsData::doPreRelease(QMap<QString, QString> editMap)
{
    TSqlQuery query;
    QJsonObject jsonObj;
    QJsonArray array;
    ...
    query.prepare("INSERT INTO public.release_mgmt(obj_sname, ac_classes, pc_classes, cat_class, country, lang, doc_type, rel_requester, relrequest_date, ci_date, rel_status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, now(), now(), 'pre-release')");
...
QJsonArray StandardsData::upPrelease(int id)
{
    TSqlQuery query;
    QJsonObject jsonObj;
    QJsonArray array;
    ...
    query.prepare("UPDATE public.standards_data SET spec_release='pre-release' WHERE id = ?");
    query.addBindValue(id);
