...
class StandardsData extends HTMLElement 
{
	constructor() 
	{
		super();
	...
	connectedCallback() 
	{
		this.getStandardsStatistic();
	}

	getStandardsStatistic() 
	{
	  return new Promise((res, rej) => {
		fetch('/standardsdata/getStatistics')
		  .then(data => data.json())
		  .then((json) => {
			this.renderStandardsStatistic(json);
			res();
		  })
	...
	renderStandardsStatistic(data) 
	{
		...
		node = document.createElement("p");
        if(Obj[i].count_Stdwaste > 0) {node.setAttribute("style", "color: Tomato");}
        else{node.setAttribute("style", "color: green");}
		textnode = document.createTextNode("gelöschte Bausteine: " + Obj[i].count_Stdwaste);
		node.appendChild(textnode);
		cell1.appendChild(node);
...
customElements.define("standards-data", StandardsData);
