...
{   
	...
	setTimeout(() => { 
		...
		// document.getElementById("redmsg").innerHTML = "pbreak: " + pcount.length;
		let counter = 1;
    ...
    }).then(function (json) 
    {
		...
		for (i in obj) 
        {
			...
			else if(obj[i].spec_release == 'pre-release')
			{
				...
				section_mnu += '<span title="pre-released" class="noprint w3-green" id="relBtn_' + obj[i].id + '"><img src="/Icons/emoji-smile.svg" alt="pre-released"></span>';
			}
...
function doprelDoBtn()
{
	...
	for(let i=0; i < prereleaseArr.length; i++)
	{
		...
		fetch("/annexdata/upPrelease", { body: data, method: "post" })
		.then(data => data.json())
		.then((json) => 
		{
			if(json[0].ERROR == "0")
			...
			{
				console.error('upPrelease NOK: ' + json[0].errMsg + ' ' + json[0].query);
			}
		})
	...
	data = new URLSearchParams([["obj_sname", stdObj.obj_sname], ["cat_class", stdObj.cat_class], ["country", stdObj.country], ["lang", stdObj.lang], ["ac_classes", stdObj.ac_classes], ["pc_classes", stdObj.pc_classes], ["relrequest_date", stdObj.relrequest_date]]);
		fetch("/annexdata/doPreRelease", { body: data, method: "post" })
		.then(data => data.json())
		.then((json) => 
		{
			if(json[0].ERROR == "0")
			{
				document.getElementById("greenmsg").innerHTML = `<b>${stdObj.obj_sname} auf pre-release gesetzt.</b>`;
				document.getElementById("greennotice").innerHTML = "";
			}
			else
			{
				console.error('doPreRelease NOK: ' + json[0].errMsg + ' ' + json[0].query);
			}
		})
...
function getSpec(id, sect, stdObj)
{
    ...
    }).then(function (json) 
    {
        ...
        repl = repl.replace(/{{lenkinfo_obj_name}}/g, stdObj.obj_sname);
        ...
        switch(stdObj.obj_sname) 
		{
			...
			case "Annex B-3": {(stdObj.lang == "de_DE") ? repl = repl.replace(/{{obj_name}}/g, "Anhang B-3: IT-Rack SDN-Server Bebauungsvorgaben") : repl = repl.replace("{{obj_name}}", "Annex B-3: IT-Rack SDN-Server Setup Requirements"); break;}
