...
function showListStds(stdObj, cat, obj_name)
{
	...
	if(obj_name == 'General')
	{
		...
		data = new URLSearchParams([["getStdType", "show"], ["obj_sname", obj_name], ["cat_sname_en", cat], ["country", "WW"], ["lang", stdObj.lang], ["spec_active", stdObj.spec_active], ["ac_class", "0"], ["pc_class", "0"], ["spec_release", stdObj.spec_release]]);
	}
	else
	{
    ...
    }).then(function (json) 
    {
		...
		for (i in obj) 
        {
			...
			section_head.innerHTML = obj[i].spec_version + " - " + datestr; // + "<data-rnav>" + section_mnu + "</data-rnav>";
