...
function setListAnnex(obj, cat, obj_name)
{
	...
	if(obj_name == cat)
	{
		...
		data = new URLSearchParams([["getStdType", "list"], ["obj_sname", obj_name], ["cat_sname_en", cat], ["country", "WW"], ["lang", obj.lang], ["spec_active", obj.active], ["ac_class", "0"], ["pc_class", "0"], ["spec_release", obj.release]]);
	}
	else
	{
    ...
    }).then(function (json) 
    {
        ...
        header.rows[0].cells[10].setAttribute('style', 'font-weight: bold; width:160px;');
        ...
        for (i in obj) 
        {
            if(!obj[i].hasOwnProperty('ERROR'))
            {
                ...
                tbody.rows[i].cells[9].appendChild(document.createTextNode(obj[i].spec_marker));
                tbody.rows[i].insertCell(10);
                ...
                tbody.rows[i].cells[10].appendChild(a);
                a = document.createElement('a');
                ...
                tbody.rows[i].cells[10].appendChild(a);
                a = document.createElement('a');
                a.href =  '/annexdata/editor_upd/' + obj[i].id;
                a.title = 'Editor';
                a.style = 'text-decoration: none; background: none;';
