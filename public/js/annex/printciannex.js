...
{   
	...
	if(stdObj.spec_release === 'pre-released' || stdObj.spec_release === 'pre-released review')
	{
		stdObj.spec_release = 'pre-release';
	}
...
function showListStds(stdObj, cat, obj_name)
{
    ...
    }).then(function (json) 
    {
		...
		for (i in obj) 
        {
			...
			section_head.innerHTML = obj[i].spec_title + " " + obj[i].spec_version + " vom " + datestr; // + "<data-rnav>" + section_mnu + "</data-rnav>";
...
function doprelDoBtn()
{
	...
	data = new URLSearchParams([["release_id", stdObj.release_id]]);
		...
		.then((json) => 
		{
			if(json[0].ERROR == "0")
			{
				document.getElementById("greenmsg").innerHTML = `<b>${stdObj.obj_sname} auf released gesetzt.</b>`;
				document.getElementById("greennotice").innerHTML = "";
			}
			else
			{
...
function getSpec(id, sect, stdObj)
{
    ...
    }).then(function (json) 
    {
        ...
        switch(stdObj.obj_sname) 
		{
			...
			case "Annex F": {(stdObj.lang == "de_DE") ? repl = repl.replace("{{obj_name}}", "Anhang F: Anfoderungen an die Stromversorgung") : repl = repl.replace("{{obj_name}}", "Annex F: Requirements to be met on Power Supply"); break;}
