...
function delComment(id, spec_id)
{
	...
	fetch("/StandardsDataComments/remove/" + id, { body: data, method: "post" });
...
function open_txtcomments(spec_id)
{
    ...
    fetch("/StandardsDataComments/getSpecComments/" + spec_id)
...
function getLanguages(Obj)
{
	...
	document.getElementById("lang").addEventListener("click", function(){ getLang(document.getElementById("lang").value); });
}
...
{   
	...
	if(stdObj.spec_release === "draft")
	{
		document.getElementById('btnprint').style.display='block';
	}
	
	setTimeout(() => { 
		let pcount = document.querySelectorAll("data-pcount"); 
		//document.getElementById("redmsg").innerHTML = "pbreak: " + pbreak.length;
		for(i = 0; i < pcount.length; i++)
		{
			pcount[i].childNodes[0].innerHTML = pcount.length;
		}
	}, 1000);
...
function showListStds(stdObj, cat, obj_name)
{
    ...
    }).then(function (json) 
    {
		...
		{
			//article.innerHTML = `<h1 data-title="title">${obj_name} ${cat}</h1>`;
			article.innerHTML = `<h1 data-title="title">${obj_name} ${translate(cat)}</h1>`;
		}
		else
		{
			article.innerHTML = `<h1 data-title="title">${cat}</h1>`;
			if(obj_name != cat && cat != 'General')
            {
		...
		for (i in obj) 
        {
			...
			else if(obj[i].spec_release == 'pre-release')
			{
				let counter = parseInt(document.getElementById("prelCount").innerHTML);
				document.getElementById("prelCount").innerHTML = counter + 1;
				section_mnu += '<span title="pre-released" class="w3-green" id="relBtn_' + obj[i].id + '"><img src="/Icons/emoji-smile.svg" alt="pre-released"></span>';
			}
...
function setRelease(id)
{
	...
	if(css != "w3-green")
	{
		...
		document.getElementById("prelMsg").innerHTML = '&harr; Pre-Release:';
		document.getElementById("prelCount").innerHTML = counter + 1;
...
function doprelDoBtn()
{
	...
	for(let i=0; i < prereleaseArr.length; i++)
	{
		...
		.then((json) => 
		{
			if(json[0].ERROR === "0")
			{
				document.getElementById("greenmsg").innerHTML = `Baustein ${prereleaseArr[i]} auf pre-release gesetzt.`;
				document.getElementById("greennotice").innerHTML = "";
			}
			else
			{
				console.log('upPrelease NOK: ' + json[0].errMsg + ' ' + json[0].query);
			}
		})
	...
	data = new URLSearchParams([["obj_sname", stdObj.obj_sname], ["cat_class", stdObj.cat_class], ["country", stdObj.country], ["lang", stdObj.lang], ["ac_classes", stdObj.ac_classes], ["pc_classes", stdObj.pc_classes], ["relrequest_date", stdObj.relrequest_date]]);
		fetch("/StandardsData/doPreRelease", { body: data, method: "post" })
		.then(data => data.json())
		.then((json) => 
		{
			if(json[0].ERROR === "0")
			{
				document.getElementById("greenmsg").innerHTML = `${stdObj.obj_sname} auf pre-release gesetzt.`;
				document.getElementById("greennotice").innerHTML = "";
			}
			else
			{
				console.log('doPreRelease NOK: ' + json[0].errMsg + ' ' + json[0].query);
			}
		})
		.catch((error) => { document.getElementById("redmsg").innerHTML = "Fehler doPreRelease: " + error + " " + data; });
...
function getSpec(id, sect, stdObj)
{
    ...
    }).then(function (json) 
    {
        ...
        // LenkInfo
        if(stdObj.spec_release !== "released")
        {
			repl = repl.replace("{{lenkinfo_spec_release}}", `<mark style="color: red;">${stdObj.spec_release} - nicht freigegeben</mark>`);
		}
		else
		{
			repl = repl.replace("{{lenkinfo_spec_release}}", stdObj.spec_release);
		}
		repl = repl.replace("{{lenkinfo_specrelease}}", stdObj.spec_release);
        repl = repl.replace("{{lenkinfo_obj_name}}", stdObj.obj_sname);
        repl = repl.replace("{{lenkinfo_ac_class}}", stdObj.ac_class);
        repl = repl.replace("{{lenkinfo_pc_class}}", stdObj.pc_class);
        repl = repl.replace("{{lenkinfo_country}}", stdObj.country);
        repl = repl.replace("{{lenkinfo_lang}}", stdObj.lang);
        
        switch(stdObj.obj_sname) 
		{
			case "ND": repl = repl.replace("{{obj_name}}", "Network Distribution (ND)"); break;
			case "BD": repl = repl.replace("{{obj_name}}", "Building Distribution (BD)"); break;
			...
			case "BMW-DC": repl = repl.replace("{{obj_name}}", "BMW Datacenter (DC)"); break;
			case "Colo-DC": repl = repl.replace("{{obj_name}}", "Colocation Datacenter (Colo-DC)"); break;
