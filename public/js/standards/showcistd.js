...
{   
	...
	document.getElementById('imgpdf').style.backgroundColor = "";
...
function doprelDoBtn()
{
	...
	for(let i=0; i < prereleaseArr.length; i++)
	{
		...
		fetch("/StandardsData/upReleased", { body: data, method: "post" })
		.then(data => data.json())
		.then((json) => 
		{
			if(json[0].ERROR === "0")
			...
			{
				console.log('upRelease NOK: ' + json[0].errMsg + ' ' + json[0].query);
			}
		})
		.catch((error) => { document.getElementById("redmsg").innerHTML = "Fehler upRelease: " + error + " " + data; });
	}
...
function getSpec(id, sect, stdObj)
{
    ...
    }).then(function (json) 
    {
		...
		if(stdObj.ac_class == 1)
		{
			repl = repl.replace("{{GLEGACY}}", "");
		}
		else if(stdObj.ac_class == 2)
		{
